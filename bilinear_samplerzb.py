...
def bilinear_sampler_1d_v(input_images, y_offset, wrap_mode='border', name='bilinear_sampler', **kwargs):
    """
    create by zhoubo
    :param input_images:
    :param y_offset:
    ...
    def _interpolate(im, x, y):
        with tf.variable_scope('_interpolate'):
            ...
            pix_t = tf.gather(im_flat, idx_t)
            pix_b = tf.gather(im_flat, idx_b)

            weight_t = tf.expand_dims(y1_f - y, 1)
            weight_b = tf.expand_dims(y - y0_f, 1)

            return weight_t * pix_t + weight_b * pix_b

    def _transform(input_images, y_offset):
        with tf.variable_scope('transform'):
            # grid of (x_t, y_t, 1), eq (1) in ref [1]
            x_t, y_t = tf.meshgrid(tf.linspace(0.0,   _width_f - 1.0,  _width),
                                   tf.linspace(0.0 , _height_f - 1.0 , _height))
...
def bilinear_sampler_2d(input_images, x_offset, y_offset, wrap_mode='border', name='bilinear_sampler', **kwargs):
    hori_im = bilinear_sampler_1d_h(input_images, x_offset)
    return bilinear_sampler_1d_v(hori_im, y_offset)
